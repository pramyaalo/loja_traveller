...
import 'markupHolidayDestination.dart';


class EditmarkupHolidaydestination extends StatefulWidget {
  final String ID;

  EditmarkupHolidaydestination({required this.ID});
  @override
  _MyRechargePageState createState() => _MyRechargePageState();
}

class _MyRechargePageState extends State<EditmarkupHolidaydestination> {
  ...
  Future<List<ViewholidayDestinationModel>?> getLabels() async {
    List<ViewholidayDestinationModel> labelData = [];

    Future<http.Response>? futureLabels = ResponseHandler.performPost(
      "MarkupHolidayDestinationViewEdit",
      "Id=${widget.ID}",
    );
  ...
  Future<void> fetchAssetData() async {
    List<ViewholidayDestinationModel>? assets = await getLabels();

    if (assets != null && assets.isNotEmpty) {
      setState(() {

        DestinationName = assets.first.holidayCityName ?? '';
  ...
  Future<List<DestinationNameModel>> fetchAutocompleteData(String empName) async {
    final url =
        'https://traveldemo.org/travelapp/traveller.asmx/MarkupHolidayDestinationname?empName=$empName&empName1=&PowerStats=&PowerSt=&Power=&UID=35510b94-5476-TDemoTraveller-a2e3-2e9779';
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    child: Autocomplete<DestinationNameModel>(
                      ...
                      '${option.latinFullName}, ${option.countryCode}, ${option.locationId}',
                      onSelected: (DestinationNameModel? selectedOption) {
                        if (selectedOption != null) {
                          print(
                              'Selected: ${selectedOption.latinFullName} (${selectedOption.countryCode})');
                          setState(() {
                            CountryCode = selectedOption.countryCode;

                            DestinationName = selectedOption.latinFullName;
                            // Update text field
                          });
                        }
                      },
                ...
                onPressed: () async {
                  if (DestinationName.isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text("Please enter the Destination Name")),
                    );
                    return;
                  }

                  if (selectedApiValue == null || selectedApiValue.toString().isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text("Please select a Markup Type")),
                    );
                    return;
                  }

                  if (MarkupValueController.text.trim().isEmpty) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text("Please enter a Markup Value")),
                    );
                    return;
                  }
                  showDialog(
                  ...
                  String apiUrl = "MarkupHolidayDestinationSet";
                  Map<String, String> apiParams = {
                    "Id": widget.ID,
                    "UserTypeId": userTypeID,
                    "UserId": userID,
                    "HolidayCityName": DestinationName,
                    "HolidayCityId": '',
                    ...
                    if (jsonResponse['Table'] != null &&
                      ...
                      if (totalPage != null && totalPage > 0) {
                        ...
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                            builder: (context) => markupHolidayDestination(),
                          ),
                        );
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
