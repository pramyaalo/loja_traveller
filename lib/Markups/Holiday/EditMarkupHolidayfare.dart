...
import 'Markupholidayfare.dart';
import 'ViewmarkupHolidayfareModel.dart';


class EditMarkupHolidayfare extends StatefulWidget {
  final String ID;

  EditMarkupHolidayfare({required this.ID});
  @override
  _MyRechargePageState createState() => _MyRechargePageState();
}

class _MyRechargePageState extends State<EditMarkupHolidayfare> {
  ...
  Future<List<ViewmarkupHolidayfareModel>?> getLabels() async {
    List<ViewmarkupHolidayfareModel> labelData = [];

    Future<http.Response>? futureLabels = ResponseHandler.performPost(
      "MarkupHolidayFareViewEdit",
      "Id=${widget.ID}",
    );
    ...
    return await futureLabels.then((value) {
      ...
      try {
        ...
        for (int i = 0; i < list.length; i++) {
          ViewmarkupHolidayfareModel lm = ViewmarkupHolidayfareModel.fromJson(list[i]);
          labelData.add(lm);
        }
      } catch (error) {
  ...
  Future<void> fetchAssetData() async {
    List<ViewmarkupHolidayfareModel>? assets = await getLabels();
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  String apiUrl = "MarkupHolidayFareSet";
                    ...
                    if (jsonResponse['Table'] != null &&
                      ...
                      if (totalPage != null && totalPage > 0) {
                        ...
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                            builder: (context) => Markupholidayfare(),
                          ),
                        );
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
