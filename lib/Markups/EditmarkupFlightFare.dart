...
import 'MarkupFlightFare.dart';
import 'ViewMarkupFlightFareModel.dart';

class EditmarkupFlightFare extends StatefulWidget {
  final String ID;

  EditmarkupFlightFare({required this.ID});
  @override
  _MyRechargePageState createState() => _MyRechargePageState();
}

class _MyRechargePageState extends State<EditmarkupFlightFare> {
  ...
  Future<List<ViewMarkupFlightFareModel>?> getLabels() async {
    List<ViewMarkupFlightFareModel> labelData = [];

    Future<http.Response>? futureLabels = ResponseHandler.performPost(
      "MarkupFlightFareViewEdit",
      "Id=${widget.ID}",
    );
    ...
    return await futureLabels.then((value) {
      ...
      try {
        ...
        for (int i = 0; i < list.length; i++) {
          ViewMarkupFlightFareModel lm = ViewMarkupFlightFareModel.fromJson(list[i]);
          labelData.add(lm);
        }
      } catch (error) {
  ...
  Future<void> fetchAssetData() async {
    List<ViewMarkupFlightFareModel>? assets = await getLabels();
                ...
                children: [
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        String apiUrl = "MarkupFlightFareSet";
                              ...
                              if (tableData.isNotEmpty && tableData[0]['totalpage'] != null && tableData[0]['totalpage'] > 0) {
                                setState(() {
                                  Navigator.pushReplacement(
                                    context,
                                    MaterialPageRoute(builder: (context) => MarkupFlightFare()),
                                  );
                                });
                              } else {
