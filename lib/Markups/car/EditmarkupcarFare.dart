...
class EditmarkupcarFare extends StatefulWidget {
  final String ID;

  EditmarkupcarFare({required this.ID});
  @override
  _MyRechargePageState createState() => _MyRechargePageState();
}

class _MyRechargePageState extends State<EditmarkupcarFare> {
  ...
  Future<List<ViiewMarkupcarFareModel>?> getLabels() async {
    List<ViiewMarkupcarFareModel> labelData = [];

    Future<http.Response>? futureLabels = ResponseHandler.performPost(
      "MarkupCarFareViewEdit",
      "Id=${widget.ID}",
    );
  ...
  Future<void> fetchAssetData() async {
    List<ViiewMarkupcarFareModel>? assets = await getLabels();
