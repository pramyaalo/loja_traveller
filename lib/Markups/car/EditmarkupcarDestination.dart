...
import 'CarDestinationModel.dart';
import 'MarkupCarDestination.dart';
import 'ViewMarkupcarDestinationModel.dart';


class EditmarkupcarDestination extends StatefulWidget {
  final String ID;

  EditmarkupcarDestination({required this.ID});
  @override
  _MyRechargePageState createState() => _MyRechargePageState();
}

class _MyRechargePageState extends State<EditmarkupcarDestination> {
  ...
  Future<List<ViewMarkupcarDestinationModel>?> getLabels() async {
    List<ViewMarkupcarDestinationModel> labelData = [];

    Future<http.Response>? futureLabels = ResponseHandler.performPost(
      "MarkupCarDestinationViewEdit",
      "Id=${widget.ID}",
    );
    ...
    return await futureLabels.then((value) {
      ...
      try {
        ...
        for (int i = 0; i < list.length; i++) {
          ViewMarkupcarDestinationModel lm = ViewMarkupcarDestinationModel.fromJson(list[i]);
          labelData.add(lm);
        }
      } catch (error) {
  ...
  Future<void> fetchAssetData() async {
    List<ViewMarkupcarDestinationModel>? assets = await getLabels();

    if (assets != null && assets.isNotEmpty) {
      setState(() {
        SelectionValue = assets.first.fromAirportName ?? ''; // Ensure it is not null
        FinalID = assets.first.fromAirport ?? '';
        Selectionvalue1 = assets.first.toAirportName ?? ''; // Ensure it is not null
        FinalID1 = assets.first.toAirport ?? '';
    ...
    print("AirportShortFrom"+AirportShortFrom);
      });
    }
  }
  ...
  Future<List<CarDestinationModel>> fetchAutocompleteData(
      String empName)
  async {
    final url =
        'https://traveldemo.org/travelapp/traveller.asmx/MarkupCarDestinationName?Name=$empName&UID=35510b94-5476-TDemoTraveller-a2e3-2e9779';
    ...
    if (response.statusCode == 200) {
      ...
      if (decodedData is Map<String, dynamic> &&
        ...
        return dataList
            .map((data) =>
            CarDestinationModel.fromJson(data as Map<String, dynamic>))
            .toList();
      } else {
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Padding(
            ...
            child: Container(
              ...
              child: Autocomplete<CarDestinationModel>(
                optionsBuilder: (TextEditingValue textEditingValue) async {
                  if (textEditingValue.text.isEmpty) {
                    return const Iterable<CarDestinationModel>.empty();
                  }
                  return await fetchAutocompleteData(textEditingValue.text);
                },
                displayStringForOption: (CarDestinationModel option) =>
                '${option.name}, ${option.id}, ${option.municipality}',
                onSelected: (CarDestinationModel? selectedOption) {
                ...
                fieldViewBuilder: (context, controller, focusNode, onFieldSubmitted) {
                  controller.text = SelectionValue; // Set the initial value
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  String apiUrl = "MarkupCarDestinationSet";
                    ...
                    if (jsonResponse['Table'] != null &&
                      ...
                      if (totalPage != null && totalPage > 0) {
                        ...
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                            builder: (context) => MarkupCarDestination(),
                          ),
                        );
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
