...
import 'Markupcardate.dart';
import 'ViewmarkupcarDateModel.dart';
 
class EditmarkupCarDate extends StatefulWidget {
  final String ID;

  EditmarkupCarDate({required this.ID});
  @override
  _MyRechargePageState createState() => _MyRechargePageState();
}

class _MyRechargePageState extends State<EditmarkupCarDate> {
  ...
  Future<List<ViewmarkupcarDateModel>?> getLabels() async {
    List<ViewmarkupcarDateModel> labelData = [];

    Future<http.Response>? futureLabels = ResponseHandler.performPost(
      "MarkupCareDateViewEdit",
      "Id=${widget.ID}",
    );
    ...
    return await futureLabels.then((value) {
      ...
      try {
        ...
        for (int i = 0; i < list.length; i++) {
          ViewmarkupcarDateModel lm = ViewmarkupcarDateModel.fromJson(list[i]);
          labelData.add(lm);
        }
      } catch (error) {
  ...
  Future<void> fetchAssetData() async {
    List<ViewmarkupcarDateModel>? assets = await getLabels();
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  String apiUrl = "MarkupCarDateSet";
