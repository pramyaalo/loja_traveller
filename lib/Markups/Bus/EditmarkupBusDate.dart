...
import 'MarkupBusDate.dart';
import 'ViewMarkupBusDateModel.dart';
 
class EditmarkupBusDate extends StatefulWidget {
  final String ID;

  EditmarkupBusDate({required this.ID});
  @override
  _MyRechargePageState createState() => _MyRechargePageState();
}

class _MyRechargePageState extends State<EditmarkupBusDate> {
  ...
  Future<List<ViewMarkupBusDateModel>?> getLabels() async {
    List<ViewMarkupBusDateModel> labelData = [];

    Future<http.Response>? futureLabels = ResponseHandler.performPost(
      "MarkupBusDateViewEdit",
      "Id=${widget.ID}",
    );
    ...
    return await futureLabels.then((value) {
      ...
      try {
        ...
        for (int i = 0; i < list.length; i++) {
          ViewMarkupBusDateModel lm = ViewMarkupBusDateModel.fromJson(list[i]);
          labelData.add(lm);
        }
      } catch (error) {
  ...
  Future<void> fetchAssetData() async {
    List<ViewMarkupBusDateModel>? assets = await getLabels();
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  String apiUrl = "MarkupBusDateSet";
