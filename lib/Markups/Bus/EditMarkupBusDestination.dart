...
import 'BusDestinationModel.dart';
import 'MarkupBusDestination.dart';
import 'ViewMarkupBusDestinationModel.dart';
 


class EditMarkupBusDestination extends StatefulWidget {
  final String ID;

  EditMarkupBusDestination({required this.ID});
  @override
  _MyRechargePageState createState() => _MyRechargePageState();
}

class _MyRechargePageState extends State<EditMarkupBusDestination> {
  ...
  Future<List<ViewMarkupBusDestinationModel>?> getLabels() async {
    List<ViewMarkupBusDestinationModel> labelData = [];

    Future<http.Response>? futureLabels = ResponseHandler.performPost(
      "MarkupBusDestinationViewEdit",
      "Id=${widget.ID}",
    );
    ...
    return await futureLabels.then((value) {
      ...
      try {
        ...
        for (int i = 0; i < list.length; i++) {
          ViewMarkupBusDestinationModel lm = ViewMarkupBusDestinationModel.fromJson(list[i]);
          labelData.add(lm);
        }
      } catch (error) {
  ...
  Future<void> fetchAssetData() async {
    List<ViewMarkupBusDestinationModel>? assets = await getLabels();

    if (assets != null && assets.isNotEmpty) {
      setState(() {
        SelectionValue = assets.first.fromBusStation ?? ''; // Ensure it is not null
        Selectionvalue1 = assets.first.toBusStation ?? '';
  ...
  Future<List<BusDestinationModel>> fetchAutocompleteData(String empName) async {
    final url =
        'https://traveldemo.org/travelapp/traveller.asmx/MarkupBusDestinationName?empName=$empName&empName1=&PowerStats=&PowerSt=&Power=&UID=35510b94-5476-TDemoTraveller-a2e3-2e9779';
    ...
    if (response.statusCode == 200) {
      ...
      return decodedData
          .map((data) => BusDestinationModel.fromJson(data as Map<String, dynamic>))
          .toList();
    } else {
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Padding(
            ...
            child: Container(
              ...
              child: Autocomplete<BusDestinationModel>(
                optionsBuilder: (TextEditingValue textEditingValue) async {
                  if (textEditingValue.text.isEmpty) {
                    return const Iterable<BusDestinationModel>.empty();
                  }
                  return await fetchAutocompleteData(textEditingValue.text);
                },
                displayStringForOption: (BusDestinationModel option) =>
                '${option.cityName}, ${option.cityId}',
                onSelected: (BusDestinationModel? selectedOption) {
                  if (selectedOption != null) {
                    print('Selected: ${selectedOption.cityName} (${selectedOption.cityId})');
                    setState(() {
                      FinalID = selectedOption.cityId;
                      SelectionValue = selectedOption.cityName;
                    });
                  }
                },
                fieldViewBuilder: (context, controller, focusNode, onFieldSubmitted) {
                  controller.text = SelectionValue; // Set text from SelectionValue
              ...
              child: Autocomplete< BusDestinationModel>(
                ...
                onSelected: ( BusDestinationModel? selectedOption) {
                  if (selectedOption != null) {
                    print('Selected: ${selectedOption.cityName} (${selectedOption.cityId})');
                    setState(() {
                      FinalID1 = selectedOption.cityId;
                      Selectionvalue1 = selectedOption.cityName;
                    });
                  }
                },
                fieldViewBuilder:
                    (context, controller, focusNode, onFieldSubmitted) {
                      controller.text = Selectionvalue1;
                  return Container(
                    width: double.infinity, // Ensures full width
                    decoration: BoxDecoration(
                      border: Border.all(color: Colors.grey), // Grey border
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  String apiUrl = "MarkupBusDestinationSet";
                  Map<String, String> apiParams = {
                    "Id": widget.ID,
                    "UserTypeId": userTypeID,
                    "UserId": userID,
                    "FromBusStation": SelectionValue,
                    "ToBusStation": Selectionvalue1,
                    "FromBusStationId": FinalID,
                    "ToBusStationId": FinalID1,
                    ...
                    if (jsonResponse['Table'] != null &&
                      ...
                      if (totalPage != null && totalPage > 0) {
                        ...
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                            builder: (context) => MarkupBusDestination(),
                          ),
                        );
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
