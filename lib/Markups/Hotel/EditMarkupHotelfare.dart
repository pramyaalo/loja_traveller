...
import 'MarkupHotelFare.dart';
import 'ViewMarkupHotelFareModel.dart';
 

class EditMarkupHotelfare extends StatefulWidget {
  final String ID;

  EditMarkupHotelfare({required this.ID});
  @override
  _MyRechargePageState createState() => _MyRechargePageState();
}

class _MyRechargePageState extends State<EditMarkupHotelfare> {
  ...
  Future<List<ViewMarkupHotelFareModel>?> getLabels() async {
    List<ViewMarkupHotelFareModel> labelData = [];

    Future<http.Response>? futureLabels = ResponseHandler.performPost(
      "MarkupHotelFareViewEdit",
      "Id=${widget.ID}",
    );
    ...
    return await futureLabels.then((value) {
      ...
      try {
        ...
        for (int i = 0; i < list.length; i++) {
          ViewMarkupHotelFareModel lm = ViewMarkupHotelFareModel.fromJson(list[i]);
          labelData.add(lm);
        }
      } catch (error) {
  ...
  Future<void> fetchAssetData() async {
    List<ViewMarkupHotelFareModel>? assets = await getLabels();
          ...
          Padding(
            ...
            child: SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  if (double.tryParse(FromfareController.text.trim()) == null ||
                      double.tryParse(ToFareController.text.trim()) == null ||
                      double.tryParse(MarkupValueController.text.trim()) == null) {
                  ...
                  String apiUrl = "MarkupHotelFareSet";
                    ...
                    if (jsonResponse['Table'] != null &&
                      ...
                      if (totalPage != null && totalPage > 0) {
                        ...
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                            builder: (context) => MarkupHotelFare(),
                          ),
                        );
                      } else {
                        ScaffoldMessenger.of(context).showSnackBar(
