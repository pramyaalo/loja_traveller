...
class EDitHoteldate extends StatefulWidget {
  final String ID;

  EDitHoteldate({required this.ID});
  @override
  _MyRechargePageState createState() => _MyRechargePageState();
}

class _MyRechargePageState extends State<EDitHoteldate> {
  ...
  Future<List<ViewMarkupHotelDateModel>?> getLabels() async {
    List<ViewMarkupHotelDateModel> labelData = [];

    Future<http.Response>? futureLabels = ResponseHandler.performPost(
      "MarkupHotelDateViewEdit",
      "Id=${widget.ID}",
    );
  ...
  Future<void> fetchAssetData() async {
    List<ViewMarkupHotelDateModel>? assets = await getLabels();
