...
class MultiCityBooking extends StatefulWidget {
  ...
  const MultiCityBooking(
      {super.key,
        ...
        required this.StopCountFourth,
        required this.RowTypeFirst,
        required this.RowTypeSecond,
        required this.MainRowNumberSecond,
...
// Infant 5
  ...
  void parseFareSummary(String xmlResponse) {
    ...
    final fareSummaryJson = strings.last.text;
    final List<dynamic> fareSummaryList = jsonDecode(fareSummaryJson);
  ...
  String Final_DepartureFirstDate = '',
      Final_DepartureSecondDate = '',
      Final_DepartureThirdDate = '',
...
// Only for TP, send flightDetailsList and empty FareIndexID
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // values received from previous page
        final stopCountFirst = widget.StopCountFirst?.toString();
        final rowTypeFirst = widget.RowTypeFirst?.toString().toLowerCase();
        final stopCountSecond = widget.StopCountSecond?.toString();
        final rowTypeSecond = widget.RowTypeSecond?.toString().toLowerCase();
        final mainRowNumberSecond = widget.MainRowNumberSecond?.toString();

        for (var flight in parsedJson) {
          final stopCount = flight['StopCount']?.toString();
          final rowType = flight['RowType']?.toString().toLowerCase();
          final mainRowNumber = flight['MainRowNumber']?.toString();

          // எத்தனை rows அந்த mainRowNumberக்கு இருக்குனு check
          final sameMainRows = parsedJson
              .where((f) => f['MainRowNumber']?.toString() == mainRowNumber)
              .toList();

          if (stopCount == "0") {
            if (rowType == "mainrow" && sameMainRows.length == 1) {
              // ✅ Only one row with this MainRowNumber → take MainRow
              finalResultFlightData.add(flight);
            } else if (rowType == "subrow" && sameMainRows.length > 1) {
              // ✅ Multiple rows with same MainRowNumber → take SubRow only
              finalResultFlightData.add(flight);
            }
          } else {
            if (rowType == "subrow") {
              // ✅ StopCount != 0 → always prefer SubRow
              finalResultFlightData.add(flight);
            }
          }
        }

        print("🎯 Final grouped flights = ${finalResultFlightData.length}");
  ...
  final Set<String> displayedMainRows = {};
