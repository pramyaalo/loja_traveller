...
class _MultiCityFlightsListState extends State<MultiCityFlightsList> {
  ...
  String formatDate(String date) {
    if (date.isEmpty) return ""; // if no date selected
    DateTime parsedDate = DateTime.parse(date); // convert String → DateTime
    return DateFormat('dd-MM-yyyy').format(parsedDate); // format DateTime → String
  }
  Map<String, bool> airlineCheckboxes = {};
  Future<void> sendMultiWayFlightSearchRequest(
      Map<String, dynamic> filters) async {
    String fin_date1 = formatDate(widget.departDate1);
    String fin_date2 = formatDate(widget.departdate2);
    String fin_date3 = formatDate(widget.departDate3);
    String fin_date4 = formatDate(widget.departdate4);
    ...
    var requestBody = {
      ...
      'Class':'2',
      'TripType':'Multiway',
      'Origin1': widget.origin1 ?? '',
      'Destination1': widget.destination1 ?? '',
      'DepartDate1': fin_date1,
      'Origin2': widget.origin2 ?? '',
      'Destination2': widget.destination2 ?? '',
      'DepartDate2': fin_date2,
      'Origin3': widget.origin3 ?? '',
      'Destination3': widget.destination3 ?? '',
      'DepartDate3': fin_date3,
      'Origin4': widget.origin4 ?? '',
      'Destination4': widget.destination4 ?? '',
      'DepartDate4': fin_date4,
    };

    print('📤 Sending Request: $requestBody');

    final url = Uri.parse(
        'https://lojatravel.com/app/b2badminapi.asmx/FlightSearch_Multiway');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final parsedData =
        Responsehandler.parseData(response.body);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          Image.asset(
            'assets/images/lastminutelogo.png',
            width: 100,
            height: 50,
          ),
          SizedBox(width: 10,),
                                  ...
                                  Container(
                                    child: Row(
                                      ...
                                      children: [
                                        ...
                                        GestureDetector(
                                          onTap: () {
                                            final bookingAPI = myResult[index]['BookingAPI'] ?? '';

// எப்பவுமே filter பண்ணும் (BookingAPI == "TP" condition இல்லாமல்)
                                            final matchingRows = fullmyResult.where((item) {
                                              bool isMatch = item['MainRowNumberFirst'] ==
                                                  (myResult[index]['MainRowNumberFirst'] ?? '') &&
                                                  item['MainRowNumberSecond'] ==
                                                      (myResult[index]['MainRowNumberSecond'] ?? '');

                                              final mainRowThird = myResult[index]['MainRowNumberThird'] ?? '';
                                              final mainRowFourth = myResult[index]['MainRowNumberFourth'] ?? '';

                                              if (mainRowThird.isNotEmpty) {
                                                isMatch = isMatch && item['MainRowNumberThird'] == mainRowThird;
                                              }
                                              if (mainRowFourth.isNotEmpty) {
                                                isMatch = isMatch && item['MainRowNumberFourth'] == mainRowFourth;
                                              }
                                              ...
                                              MaterialPageRoute(
                                                builder: (context) =>
                                                    MultiCityBooking(
                                                      ...
                                                      index]
                                                      ['StopCountThird'],
                                                      StopCountFourth: myResult[
                                                      index]
                                                      ['StopCountFourth'],
                                                      RowTypeFirst: myResult[
                                                      index]['RowTypeFirst']?.toString(),
                                                      RowTypeSecond: myResult[
                                                      index]['RowTypeSecond']?.toString(),
                                                      MainRowNumberSecond: myResult[
                                                      index]['MainRowNumberSecond']?.toString(),
                                                      TotalPrice:
                                                      myResult[index]
                                                      ['TotalPrice'],
                                                      TokenValue:
...
class Responsehandler {
  static List<dynamic> parseData(String xmlString) {
    final document = XmlDocument.parse(xmlString);

    // Extract all <string> nodes
    final stringNodes = document.findAllElements('string');

    List<dynamic> parsedData = [];

    for (var node in stringNodes) {
      final rawJson = node.text.trim();
      try {
        final decoded = json.decode(rawJson);

        if (decoded is List) {
          parsedData.addAll(decoded); // If it's a list of objects
        } else {
          parsedData.add(decoded); // Single object
        }
      } catch (e) {
