...
class _OneWayBookingState extends State<AddChildScreen> {
  ...
  void initState() {
    ...
    if (widget.childIndex >= 0 && widget.childIndex < widget.childrenList.length) {
      ...
      Documentype_controller.text = adult['documentType'] ?? ''; // Handle null values
  ...
  void _saveAdult() async {
    try {
      ...
      bool nameExists = mutablechildrensList.any((adult) {
        return adult['firstName'] == firstName && adult['surname'] == surname &&
            (widget.isEdit == 'Add' || (adult['id'] != mutablechildrensList[widget.childIndex]['id']));
      });
      ...
      if (widget.isEdit == 'Edit' && widget.childIndex >= 0 && widget.childIndex < mutablechildrensList.length) {
        // Retrieve the ID of the adult we are updating
        int? adultId = mutablechildrensList[widget.childIndex]['id'];
        if (adultId != null) {
          // Update existing adult using the correct ID
          await dbHelper.updateChildrens(adultId, adultData);
          ...
          if (updatedAdult != null) {
            setState(() {
              ...
              mutablechildrensList[widget.childIndex] = updatedAdult;
            });
        ...
        await dbHelper.insertChildrens(adultData);
        Fluttertoast.showToast(
      ...
      Navigator.pop(context, mutablechildrensList); // Pass the updated list back
    } catch (e) {
