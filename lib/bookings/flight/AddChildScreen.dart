...
class AddChildScreen extends StatefulWidget {
  ...
  @override
  State<AddChildScreen> createState() => _OneWayBookingState();
}

class _OneWayBookingState extends State<AddChildScreen> {
  bool isLoading = false;
  int Status = 2;
  late List<Map<String, dynamic>> mutablechildrensList;
  ...
  void initState() {
    ...
    mutablechildrensList = List.from(widget.childrenList);
    ...
    if (widget.childIndex >= 0 && widget.childIndex < widget.childrenList.length) {
      ...
      IssueDateController.text=adult['issueDate']??'';
  ...
  void _resetDatabaseOnce() async {
    final dbHelper = ChildrenDatabaseHelper.instance;
    await dbHelper.deleteDatabaseFile();
    await dbHelper.reopenDatabase();
    print("Database deleted and recreated with latest structure");
  }
  void _saveAdult() async {
    ...
    try {
      ...
      String documentType = selectedDocumentType ?? Documentype_controller.text.trim();
      ...
      // Duplicate check
      bool nameExists = mutablechildrensList.any((child) {
        return child['firstName'] == firstName &&
            child['surname'] == surname &&
            (widget.isEdit == 'Add' ||
                (child['id'] != mutablechildrensList[widget.childIndex]['id']));
      });
      ...
      if (widget.isEdit == 'Edit' &&
          widget.childIndex >= 0 &&
          widget.childIndex < mutablechildrensList.length) {
        int? childId = mutablechildrensList[widget.childIndex]['id'];
        if (childId != null) {
          await dbHelper.updateChildrens(childId, childData);

          Map<String, dynamic>? updatedChild = await dbHelper.fetchAdultData(childId);
          if (updatedChild != null) {
            setState(() {
              mutablechildrensList[widget.childIndex] = updatedChild;
            });
      ...
      // Add
        ...
        await dbHelper.insertChildrens(childData);
        Fluttertoast.showToast(
      ...
      Navigator.pop(context, mutablechildrensList);
    } catch (e) {
    ...
    if (response.statusCode == 200) {
      ...
      if (tableData != null &&
        ...
        setState(() {
          ...
          selectedDocumentType = passportInfo['PDDocument'];
                    ...
                    children: [
                      ...
                      Padding(
                        ...
                        child: Container(
                          ...
                          child: TextFormField(
                            style: TextStyle(fontWeight: FontWeight.w500),
                            controller: adultLname_controller, readOnly: widget.isEdit == 'Edit',
                            decoration: InputDecoration(
