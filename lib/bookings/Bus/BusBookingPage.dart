...
class Busbookingpage extends StatefulWidget {
  final String JsonSelectBus;
  final String selectedSeatJson;

  const Busbookingpage({super.key, required this.JsonSelectBus,required this.selectedSeatJson});

  @override
  State<Busbookingpage> createState() => _CarDetailsPageState();
}

class _CarDetailsPageState extends State<Busbookingpage> {
...
// Repeat this pattern for all other fields
  ...
  Future<void> getAdivahaCarDetails() async {
    final url = Uri.parse(
        'https://lojatravel.com/app/b2badminapi.asmx/Car2_SelecCar');

    try {
      setState(() => isLoading = true);

      /// FIX → widget.carJson is a STRING, so decode it
      final decodedCar = widget.JsonSelectBus is String
          ? json.decode(widget.JsonSelectBus)
          : widget.JsonSelectBus;

      /// Wrap inside array
      final sendJson = [decodedCar];

      print("🚀 SENDING JSON TO API:");
      print(json.encode(sendJson));

      final response = await http.post(
        url,
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: {'JsonSelectCar': json.encode(sendJson)},
      );
      ...
      if (response.statusCode == 200) {
        ...
        List carList = [];
        if (xmlStrings.isNotEmpty && xmlStrings[0].isNotEmpty) {
          carList = json.decode(xmlStrings[0]);
        }

        List fareList = [];
        if (xmlStrings.length > 1 && xmlStrings[1].isNotEmpty) {
          fareList = json.decode(xmlStrings[1]);
        }

        setState(() {
          selectedCar = carList.isNotEmpty ? carList.first : {};
          fareDetails = fareList.isNotEmpty ? fareList.first : {};
        });

        print("🚗 CAR DETAILS: $selectedCar");
        print("💰 FARE DETAILS: $fareDetails");
      }
    } catch (e) {
      print("❌ ERROR: $e");
      setState(() => isLoading = false);
  ...
  String formatDate(String raw) {
    DateTime dt = DateFormat("MM/dd/yyyy h:mm:ss a").parse(raw);
    return DateFormat("EEE, dd MMM yyyy  h:mm a").format(dt);
  }
  String formatPickupDropFull(String dateTime) {
    // API input format
    final inputFormat = DateFormat('MM/dd/yyyy h:mm:ss a');
    final dt = inputFormat.parse(dateTime);

    // Required output format
    final outputFormat = DateFormat('EEEE, MMMM dd, yyyy  h:mm a');
              ...
              selectedCar.isEmpty
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    /// 🔥 IMAGE CAROUSEL (HOTEL STYLE SAME)
                    SizedBox(
                      width: double.infinity,
                      height: 200,
                      child: CarouselSlider(
                        items: [selectedCar["ImageUrl"]].map((imageUrl) {
                        ...
                        children: [
                          ...
                          Text(
                            "${selectedCar["Seats"] ?? '-'} Seats • "
                                "${selectedCar["BagCount"] ?? '-'} suitcase(s) & holdall per vehicle",
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: Align(
                                      alignment: Alignment.centerRight, // 🔥 LEFT-CENTER
                                      child: Text(
                                        selectedCar["ReturnLocation"] ?? "-",
                                        textAlign: TextAlign.left,
                                        softWrap: true,
                                        textWidthBasis: TextWidthBasis.parent,
                                        style: const TextStyle(fontSize: 14),
                    ...
                    /// 🔥 PICKUP / DROPOFF DETAILS (CHECKIN / CHECKOUT SAME)
                    Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          Row(
                            ...
                            children: const [
                              Text("Pick-up Date",
                                  style: TextStyle(fontWeight: FontWeight.w500, color: Color(0xFF00AF80))),
                          ...
                          Row(
                            ...
                            children: [
                              Expanded(
                                child: Text(
                                  formatPickupDropFull(selectedCar["PickupDateTime"]),
                                  softWrap: true,
                                ),
                              ),
                              ...
                              Expanded(
                                child: Text(
                                  formatPickupDropFull(selectedCar["ReturnDateTime"]),
                    ...
                    /// 🔥 CAR DETAILS (ROOMS & GUESTS STYLE SAME)
                    Padding(
                      padding: const EdgeInsets.all(20),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          ...
                          Text("Vendor Code : ${selectedCar["VendorCode"] ?? '-'}"),
                          Text("No of Doors : ${selectedCar["NoOfDoors"] ?? '-'}"),
                          Text("Transmission : ${selectedCar["TransmissionType"] ?? '-'}"),

                          const SizedBox(height: 6),

                          /// AVAILABILITY (REFUNDABLE STYLE)
                          Row(
                            children: [
                              Icon(
                                selectedCar["RateAvailability"] == "Available"
                                    ? Icons.check_circle
                                    : Icons.cancel,
                                color: selectedCar["RateAvailability"] == "Available"
                                    ? Colors.green
                                    : Colors.red,
                                size: 16,
                              ),
                    ...
                    /*  Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              "${selectedCar["APICurrency"]} ${selectedCar["VisitorsRate"]}",
                              style: const TextStyle(
                                fontSize: 22,
                                fontWeight: FontWeight.bold,
                                color: Colors.green,
                  ...
                  children: [
                    ...
                    int.parse(widget.infantCount) > 0
                      ...
                      children: [
                        ...
                        : Container(),
                    int.parse(widget.infantCount) > 1
                        ? Column(
                      children: [
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                        ...
                        : Container(),
                    int.parse(widget.infantCount) > 2
                        ? Column(
                      children: [
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                        ...
                        : Container(),
                    int.parse(widget.infantCount) > 3
                        ? Column(
                      children: [
                        Padding(
                          padding: const EdgeInsets.all(8.0),
                        ...
                        : Container(),
                    int.parse(widget.infantCount) > 4
                        ? Column(
                      children: [
                        Padding(
                          padding: const EdgeInsets.all(8.0),
