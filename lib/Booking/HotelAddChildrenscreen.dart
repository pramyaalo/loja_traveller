...
import 'HotelChildrendatabasehelper.dart';
import 'HoteldatabaseHelper.dart';

class HotelAddChildrenscreen extends StatefulWidget {
  final flightDetails,
      resultFlightData,
      adultCount,
      childrenCount,
  ...
  @override
  State<HotelAddChildrenscreen> createState() => _OneWayBookingState();
}

class _OneWayBookingState extends State<HotelAddChildrenscreen> {
  bool isLoading = false;
  int Status = 2;
  late List<Map<String, dynamic>> mutablechildrenList;
  ...
  void initState() {
    super.initState();
    mutablechildrenList = List.from(widget.childrenList);
 ...
 // Ensure you import Fluttertoast
  ...
  void _saveAdult() async {
    try {
      ...
      bool nameExists = mutablechildrenList.any((adult) {
        return adult['firstName'] == firstName && adult['surname'] == surname &&
            (widget.isEdit == 'Add' || (adult['id'] != mutablechildrenList[widget.childIndex]['id']));
      });
      ...
      if (widget.isEdit == 'Edit' && widget.childIndex >= 0 && widget.childIndex < mutablechildrenList.length) {
        // Retrieve the ID of the adult we are updating
        int? adultId = mutablechildrenList[widget.childIndex]['id'];
        if (adultId != null) {
          // Update existing adult using the correct ID
          await dbHelper.updatehotelChildrens(adultId, adultData);
          ...
          if (updatedAdult != null) {
            setState(() {
              ...
              mutablechildrenList[widget.childIndex] = updatedAdult;
            });
        ...
        await dbHelper.inserthotelChildrens(adultData);
        Fluttertoast.showToast(
      ...
      Navigator.pop(context, mutablechildrenList); // Pass the updated list back
    } catch (e) {
